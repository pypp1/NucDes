...
if TS_flag == 0:
    ...
    v = m_flr/(rho*np.pi*((D_vess_int**2)-(D_barr_ext**2)-(D_shield_ext**2)+(D_shield_int**2))/4)             #m/s
...
if Disc_flag == 0:
    ...
    # ======================================
    if TS_flag == 1:
        C1_S = ((q_0S/(k_st*mu_st**2))*(np.exp(-mu_st*t_shield)-1)-(q_0S/mu_st)*((1/h_1)+(np.exp(-mu_st*t_shield)/h_1))-(T1-T1))/(t_shield+(k_st/h_1)+(k_st/h_1))
        #((q_0S/(k_st*mu_st**2))*(np.exp(-mu_st*t_shield)-1) - (q_0S/mu_st*h_1)*(1 + np.exp(-mu_st*t_shield)))/(t_shield + ((2*k_st)/h_1))
    ...
    if Mar_criterion > 5:
        ...
        if ThinTubes_flag == 1:
            ...
            if TS_flag == 1:
                p_0_fun_S = lambda Dt: 2 * Yield_stress_S * 1/Dt
